...
public abstract class PlayerMotor : MonoBehaviour
{
    ...
    private bool GroundRaycast(Vector3 direction, float distance, float offset, out FormieCollision hit, out bool snap, LayerMask layer)
    {
        var hitDistance = 0f;
        ...
        hit = new FormieCollision(hitPoint, hitNormal, hitDistance, closestCollider);
        return colliding;
    }

    private void CallContact(Collider collider)
    {
        if (collider.TryGetComponent(out FormieObject listener))
        {
